...
var State=function(){function a(){this.width=800;this.height=600;this.isMobile=Tools.isMobile();this.isDebug=this.isPause=!1;this.keySpace=this.keyRight=this.keyLeft=this.keyUp=null;this.menuData=void 0;this.user={login:"anon",progress:{}};this.game={currentWorld:0,currentLevel:0,speedX:0,speedY:0,orientation:0,fuelMax:500,fuel:500,shell:100,power:0};this.log=function(c){for(var d=[],e=0;e<arguments.length;++e)d[e-0]=arguments[e];b.isDebug&&console.log.apply(console,$jscomp.arrayFromIterable(d))};
return{width:this.width,height:this.height,isMobile:this.isMobile,isPause:this.isPause,isDebug:this.isDebug,keyUp:this.keyUp,keyLeft:this.keyLeft,keyRight:this.keyRight,keySpace:this.keySpace,game:this.game,user:this.user,menuData:this.menuData,log:this.log}}var b;return{getInstance:function(){b||(b=a());return b}}}(),Tools=Tools||{};
Tools.availableSize={width:function(a){return Math.max(document.documentElement.clientWidth,window.innerWidth||0)},height:function(a){return Math.max(document.documentElement.clientHeight,window.innerHeight||0)}};Tools.randomBetween=function(a,b){a=void 0===a?-40:a;return Math.random()*((void 0===b?40:b)-a)+a};Tools.rb=function(a){a=void 0===a?15:a;return!(Math.floor(Math.random()*a)>a/3)};
Tools.ajaxPost=function(a,b,c,d){d=void 0===d?!0:d;var e=new XMLHttpRequest;e.open("POST",a);e.addEventListener("load",function(){200<=e.status&&400>e.status?c(e.responseText):console.error(e.status+" "+e.statusText+" "+a)});e.addEventListener("error",function(){console.error("Erreur r\u00e9seau avec l'URL "+a)});d&&(e.setRequestHeader("Content-Type","application/json"),b=JSON.stringify(b));e.send(b)};
Tools.ajaxGet=function(a,b){var c=new XMLHttpRequest;c.open("GET",a);c.addEventListener("load",function(){200<=c.status&&400>c.status&&b(c.responseText)});c.addEventListener("error",function(){});c.send(null)};Tools.isMobile=function(){console.log(navigator.userAgent);var a=(new RegExp(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile Safari/gi)).test(navigator.userAgent);console.log(a);return a};
Tools.getHash=function(){var a="moon";(new RegExp(/moon|europa|titan|mars/)).test(window.location.hash.replace("#",""))&&(a=window.location.hash.replace("#",""));return a};Tools.pixiColor=function(a){return a.replace("#","0x")};
