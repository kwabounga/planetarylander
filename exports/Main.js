...
Main.prototype.update = function () {
  ...
  // get user key borad status and update Matter lander body forces
  const m = this;
  if (this.keyUp && this.keyRight && this.keyLeft) {
    if (this.keyUp.isDown) {
      Matter.Body.setVelocity(m.lander.body, {
        x: m.lander.body.velocity.x,
        y: m.lander.body.velocity.y - 0.5,
      });
    }
    if (this.keyRight.isDown) {
      Matter.Body.setVelocity(m.lander.body, {
        x: m.lander.body.velocity.x + 0.2,
  ...
  // get 'Matter' lander body states and apply to 'Pixi' lander sprite
