    ...
    if (this.state.keyUp && this.state.keyRight && this.state.keyLeft) {
        if (this.state.keyUp.isDown) {
          Matter.Body.setVelocity(m.lander.body, {
            ...
            y: m.lander.body.velocity.y - m.data.lander.physic.reactorPower,
          });
        }
        if (this.state.keyRight.isDown) {
          Matter.Body.setVelocity(m.lander.body, {
            x: m.lander.body.velocity.x + m.data.lander.physic.stabilizersPower,
            y: m.lander.body.velocity.y,
          });
          Matter.Body.setAngle(m.lander.body, m.lander.body.angle + (m.data.lander.physic.stabilizersPower/100));
        }
